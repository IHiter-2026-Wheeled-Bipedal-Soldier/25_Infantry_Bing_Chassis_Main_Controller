...
## Codex Workflow v6 (Hooks + Skills)

- Hooks:
  - `SessionStart` -> `.Codex/hooks/hook_session_start.sh` injects workflow context from `.Codex/hooks/workflow-guide.md`.
  - `UserPromptSubmit` -> `.Codex/hooks/hook_prompt_submit.sh` creates `CPST:` checkpoint and injects mandatory git policy from `.Codex/hooks/git-harness-agent-policy.md`.
  - `Stop` -> `.Codex/hooks/hook_stop.sh` creates `CPED:` checkpoint.
  - `TaskCompleted` -> `.Codex/hooks/hook_task_complete.sh` creates `TASK:` checkpoint and updates `.Codex/plan-git-SHA.json`.

- Core skills:
  - `brainstorming` -> design note
  - `create-todolist` -> plan JSON + `.Codex/plan-context.json` + `.Codex/plan-git-SHA.json`
...
- Task status constraints in `.Codex/plan-git-SHA.json`:
